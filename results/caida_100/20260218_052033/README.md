...
**Dataset:** `caida_100` | **Date:** 2026-02-18T05:25:35 | **Duration:** 302.0s | **Speed:** 5.0x

## Executive Summary

| Metric | Result | Status |
|--------|--------|--------|
| Detection F1 Score | 0.6667 | REVIEW |
| Precision | 1.0000 | PASS |
| Recall | 0.5000 | MISSED 2 attacks |
| Network TPS (all nodes combined) | 22.7 | GOOD |
...
| Parameter | Value | Description |
|-----------|-------|-------------|
| `SIMULATION_SPEED_MULTIPLIER` | 5.0x | 1.0 = real-time |
...
| Metric | Value |
|--------|-------|
| Speed Multiplier | 5.0x |
| Wall-Clock Time | 302.0s |
| Total Observations Processed | 6,847 |
| **Network TPS (all nodes combined)** | **22.7** |
| Per-Node TPS (network TPS / node count) | 0.227 |
...
> **2 attacks missed (false negatives).** Check if nodes had enough time to process all observations. Try increasing `--duration`.
...
| Transactions Created | 2,736 |
| Committed (consensus reached) | 2,736 |
...
| Treasury Balance | 9,991,413 |
| Total Distributed | 8,587 |
| Circulating Supply | 8,587 |
...
| Metric | Value |
|--------|-------|
| Messages Sent | 183,312 |
| Messages Delivered | 183,312 |
| Messages Dropped | 0 |
...
- **Missed 2 attacks**: Ensure `--duration` is long enough for all nodes to process their observations. Also check `KNOWLEDGE_WINDOW_SECONDS` (currently 480s).
- **100 nodes incomplete**: Increase `--duration` or reduce `SIMULATION_SPEED_MULTIPLIER` (currently 5.0x).
