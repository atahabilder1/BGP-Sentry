# --------------------------------------------------------------
# File: nonrpki_65012.py
# Purpose: Logic for non-RPKI node ASN 65012
# Used By:
#   - Simulates BGP announcements from ASN 65012
...
CONFIG_FILE = os.path.join(CURRENT_DIR, "config_65012.json")
...
# --------------------------------------------------------------
# Generate Announcement or Withdrawal with Wallet
# --------------------------------------------------------------
def generate_announcement():
    return {
        ...
        "wallet_address": WALLET  # ✅ Include wallet here
    }
...
# --------------------------------------------------------------
def main():
    print(f"🚀 Starting non-RPKI node for ASN {MY_ASN} with wallet {WALLET}")
    while True:
        announcement = generate_announcement()
        try:
            with open(OUTPUT_FILE, "a") as f:
                f.write(json.dumps(announcement) + "\n")
            print(f"[{datetime.now()}] ✉️  BGP {announcement['type'].upper()}: {announcement}")
