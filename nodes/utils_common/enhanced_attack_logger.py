#!/usr/bin/env python3
"""
Enhanced Attack Logger with Trust Score Integration
"""
...
class EnhancedAttackLogger:
    ...
    def __init__(self, log_dir="../shared_data/logs/"):
        ...
        self.trust_log = self.log_dir / "trust_score_changes.csv"
    ...
    def _initialize_csv_files(self):
        ...
        # Enhanced detection log with trust scores
        ...
        # Trust score changes log
        if not self.trust_log.exists():
            with open(self.trust_log, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    'Timestamp', 'AS_Number', 'Attack_Type', 'Trust_Score_Before',
                    'Trust_Score_After', 'Penalty_Applied', 'Engine_Type'
                ])
    ...
    def log_detection_result(self, announcement, detection_results, observer_asn, detection_time_ms=5):
        ...
        # Process with trust manager (this applies penalties if attacks detected)
        ...
        if detection_results['legitimate']:
            ...
            for attack in detection_results['attacks_detected']:
                ...
                if trust_penalty > 0:
                    with open(self.trust_log, 'a', newline='') as f:
                        writer = csv.writer(f)
                        writer.writerow([
                            timestamp, sender_asn, attack_type, trust_before,
                            trust_after, trust_penalty, 'reactive'
                        ])
    ...
    def run_monthly_evaluation(self):
        """Trigger monthly adaptive trust evaluation"""
        self.trust_manager.run_periodic_evaluation()
