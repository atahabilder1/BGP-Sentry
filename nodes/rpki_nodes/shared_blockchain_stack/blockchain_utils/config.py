...
class _Config:
    ...
    # ── Trusted Path Filter ──────────────────────────────────────
    # Maximum number of non-RPKI intermediate relays allowed in AS path.
    # 0 = only first-hop origin + all-RPKI relay chains (strict, 90.7% coverage)
    # 1 = allow 1 non-RPKI relay (2-hop coverage, 99.3%)
    # 2 = allow 2 non-RPKI relays (3-hop, 99.7%)
    MAX_NON_RPKI_RELAYS: int = _int("MAX_NON_RPKI_RELAYS", 1)
